...
function App() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn('Optional data loading failed:', optionalError)
          setTopics([])
  ...
  useEffect(() => {
    ...
    if (selectedTopic) {
      filtered = filtered.filter(article =>
        article.topics && article.topics.some(topic => topic.name === selectedTopic)
      )
    }

    setFilteredArticles(filtered)
  }, [articles, searchTerm, filterType, isAISearch, selectedTopic])
  ...
  const openArticle = async (article) => {
    ...
    const newBehavior = {
      ...userBehavior,
      views: [...(userBehavior.views || []), { articleId: article.id, timestamp: Date.now() }],
      lastViewed: article.id
    }
  ...
  const toggleBookmark = (articleId) => {
    ...
    const isCurrentlyBookmarked = newBookmarks.has(articleId)
    ...
    const newBehavior = {
      ...
      bookmarks: isCurrentlyBookmarked
        ? (userBehavior.bookmarks || []).filter(id => id !== articleId)
        : [...(userBehavior.bookmarks || []), articleId]
    }
    setUserBehavior(newBehavior)
    localStorage.setItem('userBehavior', JSON.stringify(newBehavior))
  }
    ...
    <div className={`min-h-screen font-sans transition-colors duration-300 ${isDark ? 'dark bg-gray-900' : 'bg-gradient-to-br from-white to-blue-50/30'}`}>
      ...
      <header className="py-12 px-4 text-center max-w-4xl mx-auto relative">
        ...
        >
          ...
          {topics.length > 0 && (
            <motion.div
              className="mt-6 bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-700"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  {showAdvancedFilters ? 'Hide' : 'Show'} Topics
                </button>
              </div>
                ...
                >
                  ...
                  {selectedTopic && (
                    <button
                      ...
                      className="px-3 py-2 rounded-lg text-sm font-medium bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-800/30 transition-all duration-200"
                    >
                      Clear Topic Filter
                    </button>
                  )}
          ...
          {personalizedRecommendations.length > 0 && (
            <motion.div
              className="mt-6 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-2xl p-6 shadow-sm border border-purple-200 dark:border-purple-800"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
